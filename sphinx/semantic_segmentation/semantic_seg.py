...
def semantic_map(img_path, ann_path, out_dir, opacity=0.4, gt=False, cmap='Spectral', verbose=True):
    ...
    with open(ann_path, 'r') as anns_json:
        ...
        for seg_ann in anns['annotations']:
            ann_corners = [int(val) for val in seg_ann['segmentation'][2:-2].split(', ')]
            ann_corners = np.reshape(np.array(ann_corners), (-1, 2))
            colour = [int(c*255) for c in color_map[seg_ann['category_id'], :]]
            sem_seg = cv2.fillPoly(sem_seg, [ann_corners], color=tuple(colour))
            if gt:
                gt_seg = cv2.fillPoly(gt_seg, [ann_corners], color=seg_ann['category_id'])
